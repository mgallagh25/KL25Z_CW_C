...
This program will control the led on the proto-board using spi from a master device
Values are written into the SPI_reg[] array from a master device.  The master sends 
three bytes per communication: 

command byte  '*' means write to SPI_reg buffer array
address byte   from 0-9.  this is the address in the SPI_reg[] buffer array
value   byte   value that is put into the the array. 
...
void SPI1_IRQHandler(void)
{
	...
	//   *****   '#'  denotes a read operation  this part not working yet!!!! ***********
